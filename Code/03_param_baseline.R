...
predict_param <- function(param_est, X_pi) {
  sapply(seq_len(K), function(k) {
    xs    <- X_pi[, k]
    Xprev <- if (k > 1) X_pi[, 1:(k - 1), drop = FALSE] else NULL
    p     <- param_est[[k]]
    switch(k,
...
param_est <- fit_param(X_pi_train)
...
param_ll_mat_test <- predict_param(param_est, X_pi_test)
