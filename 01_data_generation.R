...
#' Compute true log-densities for given data matrix
#'
#' The function reconstructs the parameters used in `Generate_iid_from_config`
#' and evaluates the logarithmic densities of each column conditional on its
#' predecessors. Returned is an `n \times K` matrix of log-densities.
#'
#' @param X   Matrix of observations as generated by `gen_samples()`
#' @param cfg Configuration list describing the conditional distributions
#' @return    Matrix of log-densities
#' @export
true_logdensity <- function(X, cfg) {
  stopifnot(is.matrix(X), is.list(cfg))
  n <- nrow(X)
  K <- ncol(X)
  ld <- matrix(NA_real_, nrow = n, ncol = K)
  for (i in seq_len(n)) {
    for (k in seq_len(K)) {
      c_k <- cfg[[k]]
      if (is.null(c_k$parm)) {
      ...
      fun <- get(paste0("d", c_k$distr), mode = "function")
      if (c_k$distr == "gamma" &&
          all(c("shape1", "shape2") %in% names(params_k))) {
        params_k <- list(shape = params_k$shape1, scale = params_k$shape2)
      }
      ...
      ld[i, k] <- do.call(fun, c(list(x = X[i, k], log = TRUE), params_k))
    }
  }
  ld
}
