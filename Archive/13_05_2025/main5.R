...
mytrtf <- function(data, mtry = max(1, floor(ncol(data) / 3)), ntree = 200L,
                   control = ctree_control(minbucket = 25, mincriterion = .95),
                   verbose = TRUE) {
  stopifnot(is.data.frame(data))
  K <- ncol(data)
  ret <- list(K = K)
  ctm1 <- ctm(Bernstein_basis(data[[1]], order = 5, ui = "increasing"))
  ret$ymod <- list(mlt(ctm1, data = data))
  pb <- txtProgressBar(min = 2, max = K, style = 3)
  ret$forests <- vector("list", K - 1)
  for (k in 2:K) {
    y <- names(data)[k]
    rhs <- paste(names(data)[1:(k - 1)], collapse = " + ")
    frml <- as.formula(paste0(y, " ~ 1 | ", rhs))
    if (verbose) cat(sprintf("[mytrtf] %d/%d fitting traforest for %s | %s\n", k, K, y, rhs))
    ymod <- Coxph(as.formula(paste0("Surv(", y, ") ~ 1")), data = data, order = 5, support = range(data[[k]]))
    ret$forests[[k - 1]] <- traforest(object = ymod, formula = frml, control = control,
                                     ntree = ntree, mtry = mtry, trace = FALSE, data = data)
...
predict.mytrtf <- function(object, newdata, type = c("logdensity", "density"), progress = FALSE) {
  type <- match.arg(type)
  stopifnot(inherits(object, "mytrtf"))
  lg <- predict(object$ymod[[1]], newdata = newdata, type = "logdensity")
  if (progress) pb <- txtProgressBar(min = 1, max = length(object$forests), style = 3)
  for (k in seq_along(object$forests)) {
    lg <- lg + predict(object$forests[[k]], newdata = newdata, type = "logdensity")
    if (progress) setTxtProgressBar(pb, k)
  }
  if (progress) close(pb)
  if (type == "density") return(exp(lg))
  lg
}

logLik.mytrtf <- function(object, newdata, ...) {
  sum(predict(object, newdata, type = "logdensity"))
}

X_all <- read.csv("X_samples_block_c.csv")
...
train <- X_all[train_idx, , drop = FALSE]
test  <- X_all[test_idx,  , drop = FALSE]

m <- mytrtf(train, verbose = TRUE)

ll <- logLik(m$ymod[[1]], newdata = test) +
      sum(sapply(m$forests, logLik, newdata = test))

ld <- predict(m, newdata = test, progress = TRUE)

stopifnot(all.equal(sum(ld), as.numeric(ll)))

if (exists("config") && exists("log_pi")) {
  attr(test, "logd") <- log_pi(as.matrix(test), cfg = config)
  plot(ld, attr(test, "logd")); abline(a = 0, b = 1)
} else {
  plot(ld, ylab = "estimated log-density"); abline(h = 0)
}