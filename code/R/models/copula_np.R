...
fit_copula_np <- function(S, seed = 42) {
  ...
  names(priors) <- as.character(lbl)

  structure(list(mode = "copula2d", classes = lbl, by_class = by_class,
                 seed = as.integer(seed), priors = priors),
            class = "copula_np")
}
...
print.copula_np <- function(x, ...) {
  ...
  priors <- if (!is.null(x$priors)) x$priors else rep(NA_real_, length(cls))
  for (y in cls) {
    ent <- x$by_class[[as.character(y)]]
    prior <- priors[as.character(y)]
    cat(sprintf("  class %s: n_tr=%d, eps=%.4g, prior=%.4f\n", y, ent$n_tr, ent$eps, prior))
  }
  invisible(x)
}
...
predict.copula_np <- function(object, newdata, y = NULL, type = c("logdensity", "logdensity_by_dim")) {
  type <- match.arg(type)
  ...
  # Helper for mixture log-sum-exp
  ...
  if (is.null(y)) {
    ...
    if (is.null(priors)) stop("copula2d: class priors missing; refit model")
    log_comp <- matrix(-Inf, N, length(classes))
    colnames(log_comp) <- as.character(classes)
    for (j in seq_along(classes)) {
      yy <- classes[[j]]
      comp <- object$by_class[[as.character(yy)]]
      log_pi <- log(priors[as.character(yy)])
      f1 <- log(pmax(kde1d::dkde1d(X[, 1], comp$m1), tiny))
      F1 <- pmin(pmax(kde1d::pkde1d(X[, 1], comp$m1), comp$eps), 1 - comp$eps)
      f2 <- log(pmax(kde1d::dkde1d(X[, 2], comp$m2), tiny))
      F2 <- pmin(pmax(kde1d::pkde1d(X[, 2], comp$m2), comp$eps), 1 - comp$eps)
      lc <- log(pmax(kdecopula::dkdecop(cbind(F1, F2), comp$cop), tiny))
      log_comp[, j] <- log_pi + f1 + f2 + lc
    }
    log_joint <- log_sum_exp(log_comp)
    if (any(!is.finite(log_joint))) stop("NA/Inf in copula mixture log density")
    LD <- cbind(log_joint / 2, log_joint / 2)
    if (type == "logdensity_by_dim") return(LD)
    return(log_joint)
  }

  yv <- as.vector(y)
  if (length(yv) != N) stop("copula2d: y must have the same length as nrow(newdata)")
