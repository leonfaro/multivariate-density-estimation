...
if (!exists("standardize_train_only")) {
  p1 <- file.path("R", "utils_standardize.R")
  if (file.exists(p1)) {
    source(p1)
  } else if (exists("root_path")) {
    p2 <- file.path(root_path, "R", "utils_standardize.R")
...
fit_ttm <- function(data, algo = c("marginal","separable","crossterm"), seed = 42, ...) {
  ...
  if (algo == "separable") {
    src <- file.path("R", "ttm_separable.R"); if (file.exists(src)) source(src)
    return(fit_ttm_separable(data, seed = seed, ...))
  }
  if (algo == "crossterm") {
    src <- file.path("R", "ttm_crossterm.R"); if (file.exists(src)) source(src)
...
predict_ttm <- function(model, X, type = c("transform", "jac_diag", "logdensity_by_dim", "logdensity")) {
  ...
  # Delegate to core
  if (!exists("ttm_forward")) source(file.path("R", "ttm_core.R"))
