...
summarise_fit <- function(param_est, X_test, ll_delta_df,
  ...
  K <- length(param_est)
  nbeta <- function(k) if (k == 1) 1 else k
  mean_p1 <- character(K)
  mean_p2 <- character(K)
  mle_p1 <- character(K)
  mle_p2 <- character(K)
  for (k in seq_len(K)) {
    ...
    params <- compute_distribution_parameters(param_est[[k]], X_prev,
                                              fam, nrow(X_test))
    mvals <- vapply(params, mean, numeric(1))
    ints <- sapply(seq_along(fam$param_names), function(j)
      param_est[[k]][1 + (j - 1) * nbeta(k)])
    mean_p1[k] <- sprintf("%.3f", mvals[1])
    mle_p1[k] <- sprintf("%.3f", ints[1])
    if (length(fam$param_names) >= 2) {
      mean_p2[k] <- sprintf("%.3f", mvals[2])
      mle_p2[k] <- sprintf("%.3f", ints[2])
    } else {
      mean_p2[k] <- "none"
      mle_p2[k] <- "none"
    }
  }
  ll_delta_df$mean_param1 <- mean_p1
  ll_delta_df$mean_param2 <- mean_p2
  ll_delta_df$mle_param1 <- mle_p1
  ll_delta_df$mle_param2 <- mle_p2
  ll_delta_df
}
