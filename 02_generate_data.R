...
#   `logd*`, `logdet_J` and `ll_true` for each observation.
...
# 3. Compute `logdet(J) = rowSums(logd)` and evaluate the log-likelihood via
#    `loglik(Z_eta, logdet(J))`.
...
train_df <- data.frame(
  ...
  logdet_J = logdet_J_train, ll_true = ll_train,
  check.names = FALSE
)
colnames(train_df) <- c(
...
test_df <- data.frame(
  ...
  logdet_J = logdet_J_test, ll_true = ll_test,
  check.names = FALSE
)
